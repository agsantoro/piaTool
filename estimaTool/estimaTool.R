...
estimaToolCosts = function(
  ...
  run[[country]]$baseline$`Muertes por Accidente Cerebrovascular por cada 100.000 habitantes que podrían evitarse` = round(outcomesEpi$dif_2,1)
  run[[country]]$baseline$`Muertes evitadas por Accidente Cerebrovascular` = round(outcomesEpi$dif_2_pob,0)
  ...
  for ( e in c("Deaths", "Events")) {
    for (c in c("Ischemic heart disease","Stroke")) {
      for (i in unique(mortality_data$age)) {
        ...
        if (e == "Deaths") {
          absolute = if (c=="Stroke") {"Muertes evitadas por Accidente Cerebrovascular"} else {"Muertes por ECI que podrían evitarse"}
        } else {
  ...
  costs_outcomes = list(
    ...
    `Costos médicos directos evitados por evento cardiovasculares (ECI y ACV)` = run[[country]]$baseline$`Total de nuevos ECI evitados` * round(`Evento de enfermedad cardiaca isquemica promedio  (***)`,3) + round(`Evento de accidente cerebrovascular`,3) * run[[country]]$baseline$`Total de nuevos ACV evitados`,
    `Diferencia de costo` = (run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo anual de consulta médica en paciente promedio (*)` + run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo farmacológico anual por paciente promedio (**)`)-(run[[country]]$baseline$`Total de nuevos ECI evitados` * round(`Evento de enfermedad cardiaca isquemica promedio  (***)`,3) + round(`Evento de accidente cerebrovascular`,3) * run[[country]]$baseline$`Total de nuevos ACV evitados`),
    `Razon incremental de costo por evento de ECI evitado` = ((run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo anual de consulta médica en paciente promedio (*)` + run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo farmacológico anual por paciente promedio (**)`)-(run[[country]]$baseline$`Total de nuevos ECI evitados` * round(`Evento de enfermedad cardiaca isquemica promedio  (***)`,3) + round(`Evento de accidente cerebrovascular`,3) * run[[country]]$baseline$`Total de nuevos ACV evitados`)) / (run[[country]]$baseline$`Total de nuevos ECI evitados`),
    ...
    `Razon incremental de costo por AVAD evitado` = ((run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo anual de consulta médica en paciente promedio (*)` + run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo farmacológico anual por paciente promedio (**)`)-(run[[country]]$baseline$`Total de nuevos ECI evitados` * round(`Evento de enfermedad cardiaca isquemica promedio  (***)`,3) + round(`Evento de accidente cerebrovascular`,3) * run[[country]]$baseline$`Total de nuevos ACV evitados`)) / (as.numeric(epi_outcomes$value[epi_outcomes$outcome == "Total de AVAD evitados"])),
    `ROI` = ((run[[country]]$baseline$`Total de nuevos ECI evitados` * round(`Evento de enfermedad cardiaca isquemica promedio  (***)`,3) + round(`Evento de accidente cerebrovascular`,3) * run[[country]]$baseline$`Total de nuevos ACV evitados`) - (run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo anual de consulta médica en paciente promedio (*)` + run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo farmacológico anual por paciente promedio (**)`))/(run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo anual de consulta médica en paciente promedio (*)` + run[[country]]$baseline$`Nueva población (N) tratada / Controlados actualmente previamente no controlados` * `Costo farmacológico anual por paciente promedio (**)`)*100
  )
